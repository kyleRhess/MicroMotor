...
int signal_currentEnableState 	= MOTOR_MODE_DISABLE;
int signal_driveDirection		= MOTOR_MODE_FORWARD;
float signal_currentPwmValue	= 0.0f;
...
void signal_SetMotorState(int state)
{
	if(state == MOTOR_MODE_ENABLE)
		WritePin(GPIOB, GPIO_DIS_PIN, 0);
	else
		WritePin(GPIOB, GPIO_DIS_PIN, 1);
...
void signal_SetMotorPWM(float speed)
{
	...
	if(signal_currentPwmValue >  100.0f)
		signal_currentPwmValue =  100.0f;
#ifdef BI_POLAR
	PWM_adjust_DutyCycle(&PWMtimer.timer, ((signal_currentPwmValue*0.5f) + 50.0f));
#else
	...
	PWM_adjust_DutyCycle(&PWMtimer.timer, fabsf(signal_currentPwmValue));
#endif
}
